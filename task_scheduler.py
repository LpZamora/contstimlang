...
class TaskScheduler():
    ...
    def _execute_sqlite(self,query,parameters=None):
        ...
        try:
            ...
            if str(e) != 'UNIQUE constraint failed: jobs.job_id': # show error, unless it's a duplicate unique job_id
    ...
    def start_job(self,job_id):
        ...
        self._execute_sqlite("DELETE FROM jobs WHERE (job_id = ? AND time_started < ? AND is_completed = 0)",(job_id_json,time_started-self.max_job_time_in_seconds))

        # try inserting new job
        success=self._execute_sqlite("INSERT INTO jobs (job_id, is_completed, time_started) VALUES (?, 0, ?)",(job_id_json,time_started))

        # when failed, provide feedback
        if success == False:
            is_completed=self._sqlite_fetchone("SELECT is_completed FROM jobs WHERE job_id = ?",(job_id_json,))
            if is_completed:
    ...
    def job_done(self,job_id,results=None):
        if results is None:
            self._execute_sqlite("INSERT OR REPLACE INTO jobs (job_id,is_completed) VALUES(?, 1)",(json.dumps(job_id),))
        else:
            self._execute_sqlite("INSERT OR REPLACE INTO jobs (job_id,is_completed,results) VALUES(?, 1,?)",(json.dumps(job_id),str(results)))

    def delete_job(self,job_id):
        self._execute_sqlite("DELETE FROM jobs WHERE (job_id = ? )",(json.dumps(job_id),))

    def delete_all_running_jobs(self,):
