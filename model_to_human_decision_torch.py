...
log_1_plus_exp_x = lambda x: torch.logsumexp(
    torch.stack((x, torch.zeros_like(x)), dim=-1), dim=1
)
...
class ModelToHumanDecision(torch.nn.Module):
    def __init__(self, parameters=None, device=None):
        ...
        for par_name, par_value in parameter_dict.items():
            if not isinstance(par_value, torch.Tensor):
                par_value = torch.tensor(
                    par_value, device=self.device, dtype=torch.float64
                )
            else:
    ...
    def forward(self, log_p_sentences1, log_p_sentences2=None):
        ...
        if log_p_sentences2 is None:
            assert (
                log_p_sentences1.shape[1] == 2
            ), "code currently implemented for sentence *pairs*"
            log_p_sentences2 = log_p_sentences1[:, 1]
            log_p_sentences1 = log_p_sentences1[:, 0]

        log_p1 = torch.as_tensor(
            log_p_sentences1, device=self.device, dtype=torch.float64
        )
        log_p2 = torch.as_tensor(
            log_p_sentences2, device=self.device, dtype=torch.float64
        )
...
class FixedWidthSquashing(ModelToHumanDecision):
    def default_parameters(self):
        ...
        parameters[
            "squashes"
        ] = 200.0  # this doesn't work very well - use minimum sentence probability instead
        parameters["gamma"] = 10.0
        return parameters
    ...
    def _f(self, log_p1, log_p2):
        ...
        width = 1.0
        log_p1_corrected = (
            width * log_1_plus_exp_x((log_p1 + squash_threshold) / width)
            - squash_threshold
        )
        log_p2_corrected = (
            width * log_1_plus_exp_x((log_p2 + squash_threshold) / width)
            - squash_threshold
        )
    ...
    def _f(self, log_p1, log_p2):
        ...
        denominator = torch.logsumexp(
            torch.stack(
                [
                    log_p1 / gamma,
                    torch.ones_like(log_p1) * eta,
                    log_p2 / gamma,
                    torch.ones_like(log_p2) * eta,
                ],
                dim=-1,
            ),
            dim=-1,
        ...
        log_p1 = (
            torch.logsumexp(
                torch.stack([log_p1 / gamma, torch.ones_like(log_p1) * eta], dim=-1),
                dim=-1,
            )
            - denominator
        )
        log_p2 = (
            torch.logsumexp(
                torch.stack([log_p2 / gamma, torch.ones_like(log_p2) * eta], dim=-1),
                dim=-1,
            )
            - denominator
        )
