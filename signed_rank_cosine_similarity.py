...
def calc_signed_rank_with_avg_ties(x):
    ...
    r[non_zero_mask] = np.sign(x[non_zero_mask]) * _rank_with_avg_ties(
        np.abs(x[non_zero_mask])
    )
    return r
...
def _calc_RAE_signed_rank_sampling(x):
    ...
    non_zero_mask = x != 0
    r = np.zeros_like(x, dtype=np.float64)
    r[non_zero_mask] = np.sign(x[non_zero_mask]) * _rank_with_random_tie_breaking(
        np.abs(x[non_zero_mask])
    )
    return r
...
def _test_calc_signed_rank_cosine_similarity_analytical_RAE(
    n_simulations=3, n_samples=100
):
        ...
        for a_magnitude in [2, 3, 5, 10]:
            for b_magnitude in [2, 3, 5, 10]:
                for i_sim in range(n_simulations):
                    ...
                    c_analytical.append(
                        calc_signed_rank_cosine_similarity_analytical_RAE(a, b)
                    )
                    c_sampling.append(
                        _calc_signed_rank_cosine_similarity_sampling_RAE(
                            a, b, n_samples=n_samples
                        )
                    )
