...
def docplex_solve(
    utility_vec, grouping1, grouping2, n_selected_per_group, maximize=True
):
    ...
    from docplex.mp.model import (
        Model,
    )  # requires also cplex -  conda install -c ibmdecisionoptimization cplex
    ...
    assert (len(groups1) * n_selected_per_group) <= n and (
        len(groups2) * n_selected_per_group
    ) <= n

    selection = mdl.binary_var_list(
        n, name="s"
    )  # this represents whether each element was "selected" or not
    ...
    mdl.add_constraints(
        [
            mdl.sum(selection[i] for i in range(n) if grouping1[i] == g)
    ...
    mdl.add_constraints(
        [
            mdl.sum(selection[i] for i in range(n) if grouping2[i] == g)
            == n_selected_per_group
            for g in groups2
        ]
    )
...
def all_sentence_lp_plot(all_sentences, figname):
    ...
    fig = plt.figure(
        figsize=(
            n_models * inch_per_subplot + inch_per_margin * 2,
            n_models * inch_per_subplot + inch_per_margin * 2,
        )
    )
    relative_margin = (
        inch_per_margin * 2 / (n_models * inch_per_subplot + inch_per_margin * 2)
    )
    gs = gridspec.GridSpec(
        nrows=n_models,
        ncols=n_models,
        figure=fig,
        left=relative_margin,
        right=1 - relative_margin,
        bottom=relative_margin,
        top=1 - relative_margin,
        wspace=0.4,
        hspace=0.4,
    )
    ...
    for c, x_model in enumerate(all_model_names):
        for r, y_model in enumerate(all_model_names):
            ...
            mask = (
                (all_sentences["m1"] == x_model) & (all_sentences["m2"] == y_model)
