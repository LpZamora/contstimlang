...
for model1_name in models:
    for step_ind in range(10):
        ...
        while get_n_lines(fname) < max_sentences:
            ...
            words1 = list(np.random.choice(vocab_cap, 1, p=vocab_cap_freqs1)) + list(
                np.random.choice(
                    vocab_low, sent_len - 1, p=vocab_low_freqs1, replace=False
                )
            )
            ...
            for samp in range(10000):
                ...
                if model1_name in ["gpt2", "rnn", "lstm"]:
                    ...
                    if cur_word1 in word1_list:
                        ...
                        if (
                            are_we_going_up
                        ):  # if the target log-probability is lower than the current log-probability,
                            highest_logprob_word_idx = np.argmax(
                                model1_word1_probs
                            )  # check first the highest log-probability word
                            if highest_logprob_word_idx != cur_word_idx:
                                ...
                                second_highest_logprob_word_idx = np.argpartition(
                                    -model1_word1_probs, 1
                                )[1]
                                initial_xs_guesses = [second_highest_logprob_word_idx]
                        else:  # going down
                            lowest_logprob_word_idx = np.argmin(
                                model1_word1_probs
                            )  # check first the lowest log-probability word
                            if lowest_logprob_word_idx != cur_word_idx:
                                ...
                                second_lowest_logprob_word_idx = np.argpartition(
                                    model1_word1_probs, 1
                                )[1]
                        ...
                        if are_we_going_up:
                            initial_xs_guesses = [
                                highest_logprob_word_idx,
                                lowest_logprob_word_idx,
                            ]
                        else:
                            initial_xs_guesses = [
                                lowest_logprob_word_idx,
                ...
                best_word1 = None
                word_iteration = 0
                while (not found_useful_replacement) and (
                    n_words_evaluated_without_loss_improvement
                    < max_n_words_to_consider_without_loss_improvement
                ):
                    word_iteration += 1
                    ...
                    if word_iteration > 1 and not found_useful_replacement:
                        ...
                        opt.update_query_result(
                            xs=[evaluation_word_idx], ys=[model1_sent1t_prob]
                        )

                        if verbose >= 3:
                            print(
                                "evaluated: {:<30} | log-prob: {:06.1f}→ {:06.1f} | ".format(
                                    cur_word1 + "→ " + word1,
                                    model1_sent1_prob,
                                    model1_sent1t_prob,
                                ),
                    ...
                    if minimum_loss is not None:
                        ...
                        if (
                            minimum_loss < best_loss_so_far
                        ):  # track relative improvement (used for stopping criterion)
                            best_word1 = word1_list[minimum_loss_word_idx]
                            best_word1_model1_sent1t_prob = opt.ys[
                                minimum_loss_word_idx
                            ].item()
                            ...
                            ):  # absolute improvement - stop word-level search
                        ...
                        if verbose >= 3:
                            print(
                                "best replacement: {:<30} | log-prob: {:06.1f}→ {:06.1f} | loss: {:06.1f}→ {:06.1f}".format(
                                    cur_word1 + "→ " + best_word1,
                                    model1_sent1_prob,
                                    best_word1_model1_sent1t_prob,
                                    cur_loss,
                                    best_loss_so_far,
                                ),
                                end="",
                            ...
                            if n_words_evaluated_without_loss_improvement > 0:
                                print(
                                    " | {:02d} words w/o loss improvement.".format(
                                        n_words_evaluated_without_loss_improvement
                                    )
                                )
                            else:
                ...
                if found_useful_replacement:
                    ...
                    assert np.isclose(
                        model1_sent1_prob, best_word1_model1_sent1t_prob, rtol=1e-3
                    )
